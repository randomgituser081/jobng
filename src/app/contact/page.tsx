...
export default function ContactPage() {
  ...
  const { phone } = useAuth(); 
  const user = { id: "65319b34-29db-4534-ab1e-81a08d7f8c89", phone };
  ...
  /* --- ATTACHMENT FEATURE COMMENTED OUT ---
  ...
  // Fetch CAPTCHA from Next.js API Route
  ...
  const loadCaptcha = useCallback(async () => {
    ...
    setCaptchaAnswer("");
    try {
      const res = await fetch(`/api/captcha?user_id=${encodeURIComponent(user.id)}`);
      ...
      if (res.ok && result.data?.success) {
        ...
        setErrorMsg("Failed to load security challenge. Please refresh.");
      }
    } catch {
      setErrorMsg("Network error loading CAPTCHA.");
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!isFormValid || !user?.id || !user?.phone) return;

    setStatus("loading");
    setErrorMsg("");

    // Combine Title and Description for the API complaint field
    const combinedComplaint = `Title: ${title.trim()}\n\nDescription: ${description.trim()}`;
    ...
    try {
      ...
      if (res.ok && result.ok) {
        ...
        // Refresh CAPTCHA on failure since it might have expired or been used
        loadCaptcha(); 
      }
    } catch {
      setStatus("error");
          ...
          <div className="bg-white/90 backdrop-blur-xl border border-slate-200/80 rounded-3xl p-6 sm:p-10 shadow-2xl lg:col-span-2">
            ...
            {status === "success" ? (
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-6">
                ...
                {/* Optional Attachment - COMMENTED OUT 
                <div className="flex flex-col gap-2">
                  <label className="text-xs font-extrabold text-slate-700 uppercase tracking-wider">
                    Attachment (Optional)
                  </label>
                  {!previewUrl ? (
                    ...
                    <div className="relative inline-flex items-center gap-4 p-3 rounded-2xl border border-slate-200 bg-white shadow-sm w-max pr-12">
                      <div className="w-12 h-12 rounded-lg bg-slate-100 overflow-hidden flex items-center justify-center shrink-0 border border-slate-200">
                        <Image src={previewUrl} alt="Preview" width={48} height={48} className="w-full h-full object-cover" />
                      </div>
                      <div className="flex flex-col">
                        <span className="text-sm font-bold text-slate-700 truncate max-w-40">{attachment?.name}</span>
                        <span className="text-xs text-slate-500">{(attachment!.size / 1024 / 1024).toFixed(2)} MB</span>
                      </div>
                      <button type="button" onClick={removeAttachment} className="absolute right-3 top-1/2 -translate-y-1/2 p-2 bg-red-50 text-red-600 rounded-full hover:bg-red-100 transition-colors">
                        <FiX />
                      </button>
                    </div>
                  )}
                  <input type="file" ref={fileInputRef} onChange={handleFileChange} accept="image/jpeg, image/png" className="hidden" />
                  {fileError && <p className="text-sm text-red-600 font-medium mt-1">{fileError}</p>}
                </div>
                ------------------------------------------------- */}

                {/* CAPTCHA Section */}
                <div className="flex flex-col gap-2 p-5 bg-slate-100/50 border border-slate-200 rounded-2xl">
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <button 
                      ...
                      className="text-slate-500 hover:text-[#00A651] transition-colors flex items-center gap-1 text-xs font-semibold"
                    >
                      <FiRefreshCw className={isFetchingCaptcha ? "animate-spin" : ""} />
                      Refresh
                    </button>
                  </div>
                  
                  {isFetchingCaptcha && !captchaQuestion ? (
                    <div className="flex items-center gap-2 text-sm text-slate-500 py-3">
                      <FiLoader className="animate-spin text-lg" /> Loading challenge...
                    </div>
                  ) : (
                    <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
                      <div className="px-4 py-3 bg-white border border-slate-300 rounded-xl font-mono text-slate-800 shadow-sm shrink-0 font-bold select-none tracking-wide">
                        {captchaQuestion || "Captcha Failed to Load"}
                      </div>
                      <input
                        name="captcha"
                        type="text"
                        ...
                        className="w-full sm:w-auto flex-1 px-4 py-3 rounded-xl border border-slate-200 bg-white text-slate-900 font-medium placeholder:text-slate-400 outline-none transition-all duration-200 focus:border-[#00A651] focus:ring-4 focus:ring-[#00A651]/15"
                      />
                    </div>
                  )}
                </div>
                ...
                <button
                  type="submit"
                  disabled={status === "loading" || !isFormValid || isFetchingCaptcha}
